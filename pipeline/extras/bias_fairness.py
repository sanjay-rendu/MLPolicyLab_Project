...
class fairness_extract(BaseOperator):
    ...
    def get_plots(df, models, house_races, senate_races):
        ...
        for race in sorted(races.keys(), reverse=True):
            if race == 'white':
                ...
                df['prob'] = baseline(df[FEATURE_COLS])
                df = df.sort_values("prob", ascending=False)
                ...
                tpr = np.sum(1-np.array(positive_pred["pred"]-positive_pred["label"]))/len(new_group)
                precision = precision_score(df["label"], df["pred"])
                base.append({"precision": precision, "fdr": fdr, "tpr": tpr, "model_name": "baseline", "race": race})
                ...
                tpr = np.sum(1-np.array(positive_pred["pred"]-positive_pred["label"]))/len(new_group)
                precision = precision_score(df["label"], df["pred"]) 
                base_model = base[base["model_name"] == "baseline"]
                base_fdr = base_model["fdr"].values[0]
                base_tpr = base_model["tpr"].values[0]
    ...
    def run(self, groups, metric="both"):
        ...
        model_grids = self.inputs["modelgrid"].read()
        all_models = []
        for grid in model_grids:
            for filename in os.listdir(grid):
                if filename.endswith(".pkl"):
                try:
                    all_models.append(pickle.load(open(lr_dir+"/"+filename, "rb")))
                except:
        ...
        get_plots(val4, all_models, house_races4, senate_races4)
